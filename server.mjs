...
import { mangaBallCatalog, mangaBallUpdates, mangaBallDetailByInternalId, mangaBallChapterByInternalId, mangaBallHome } from './lib/mangaball-provider.mjs';
...
app.get('/api/users/:username/library',publicRate,async(req,reply)=>{
  ...
  const mediaType=String(req.query?.mediaType||'MANGA').toUpperCase()==='MANGA'?'MANGA':'ANIME',table=mediaType==='MANGA'?'user_manga':'user_anime',alias=mediaType==='MANGA'?'um':'ua';
...
app.get('/api/manga/:id/chapter',{config:{rateLimit:{max:45,timeWindow:'1 minute'}}},async(req,reply)=>{
  const id=safeInt(req.params.id),number=String(req.query?.number||'').trim();
  if(!id)return reply.code(400).send({error:'INVALID_ID'});
  if(!/^[0-9A-Za-z._-]{1,40}$/.test(number))return reply.code(400).send({error:'INVALID_CHAPTER'});
  try{return await mangaBallChapterByInternalId(id,number)}catch(error){req.log.warn({err:error,id,chapter:number},'mangaball chapter failed');return reply.code(404).send({error:'MANGABALL_CHAPTER_NOT_FOUND'})}
});
...
app.put('/api/me/favorites/:mediaId',writeRate,async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const mediaId=safeInt(req.params.mediaId);if(!mediaId)return reply.code(400).send({error:'INVALID_ID'});const parsed=z.object({mediaType:z.enum(['ANIME','MANGA']).default('MANGA')}).safeParse(req.body||{});if(!parsed.success)return reply.code(400).send({error:'INVALID_INPUT'});await q('INSERT INTO user_favorites(user_id,media_id,media_type) VALUES($1,$2,$3) ON CONFLICT(user_id,media_id,media_type) DO NOTHING',[u.id,mediaId,parsed.data.mediaType]);return{ok:true,favorite:true};});
app.delete('/api/me/favorites/:mediaId',writeRate,async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const mediaId=safeInt(req.params.mediaId);if(!mediaId)return reply.code(400).send({error:'INVALID_ID'});const mediaType=String(req.query?.mediaType||'MANGA').toUpperCase()==='MANGA'?'MANGA':'ANIME';await q('DELETE FROM user_favorites WHERE user_id=$1 AND media_id=$2 AND media_type=$3',[u.id,mediaId,mediaType]);return{ok:true,favorite:false};});
...
app.put('/api/me/follows/:mediaId',writeRate,async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const mediaId=safeInt(req.params.mediaId);if(!mediaId)return reply.code(400).send({error:'INVALID_ID'});const parsed=z.object({mediaType:z.enum(['ANIME','MANGA']).default('MANGA'),notifyEpisode:z.boolean().default(true),notifyNews:z.boolean().default(true)}).safeParse(req.body||{});if(!parsed.success)return reply.code(400).send({error:'INVALID_INPUT'});const data=parsed.data;await q(`INSERT INTO user_follows(user_id,media_id,media_type,notify_episode,notify_news) VALUES($1,$2,$3,$4,$5) ON CONFLICT(user_id,media_id,media_type) DO UPDATE SET notify_episode=EXCLUDED.notify_episode,notify_news=EXCLUDED.notify_news`,[u.id,mediaId,data.mediaType,data.notifyEpisode,data.notifyNews]);return{ok:true};});
app.delete('/api/me/follows/:mediaId',writeRate,async(req,reply)=>{const u=await requireUser(req,reply);if(!u)return;const mediaId=safeInt(req.params.mediaId);if(!mediaId)return reply.code(400).send({error:'INVALID_ID'});const mediaType=String(req.query?.mediaType||'MANGA').toUpperCase()==='MANGA'?'MANGA':'ANIME';await q('DELETE FROM user_follows WHERE user_id=$1 AND media_id=$2 AND media_type=$3',[u.id,mediaId,mediaType]);return{ok:true};});
